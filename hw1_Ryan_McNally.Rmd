...
`hw1_FirstName_LastName.Rmd` will include your solutions. The file and folder names are *case-sensitive*, which means `hw1.Rmd` and `HW1.rmd` are different. *It is crucial to follow the exact folder structure and file names* in this course, so that your homework can be reproduced. The correct folder structure is
    ...
    ```
    ds202_hw1
    |-- hw1_Ryan_McNally.Rmd
    |-- hw1_Ryan_McNally.html (generated after knitting)
    ```
...
3. Write a paragraph to introduce yourself. Set your name in bold font and your major in italic. 
    ...
    Answer: Hello, my name is **Ryan McNally**. I am a junior majoring in *Software Engineering*, and minoring in Data Science. I am excited for this class becasue I have not worked with R yet and I am looking forward to getting more framiliar with it, and imporving my data science skills in general. 
...
4. Write a paragraph describing a job that you think is related to data science. Include a hyperlink and an image. Make sure your answer is in a nice format to read. 
    ...
    Answer: One job in the data science field is a buisness analyst. A buisness anayst would have the role of digging through the data of a company and providing insight and guidance for the buisness based on their finding in the trends in the data. [Business Analyst](https://www.cio.com/article/2436638/project-management-what-do-business-analysts-actually-do-for-software-implementation-projects.html) [![Foo](https://uploads-ssl.webflow.com/6095a6b50a2535d8734740bc/60a261cdcc3468fac57fd68c_business-analyst.png)](https://www.google.com/search?q=business+analyst&rlz=1C1CHBF_enUS862US862&source=lnms&tbm=isch&sa=X&ved=2ahUKEwi2j6Hm_87yAhUMCs0KHRgcAtQQ_AUoAnoECAEQBA&biw=1536&bih=696#imgrc=F_CqT7ERQyFOjM)
  
5. What data topic will you be most interested in? (Optional) Provide a source of such data set if you have.

    Answer: I have not chosen a dataset yet, but I am always looking for intersting data sets to explore. Some of the topics that I think would be interesting to explore are sports data (March Madness perhaps), gaming data (# of daily players for a game), or something topical like COVID.  
...
9. Configure your git username and user email by modifying and entering the following commands into the terminal
    ...
    ```
    git config --global user.name "Ryan McNally"
    git config --global user.email "rmcnally@iastate.edu"
    ```
